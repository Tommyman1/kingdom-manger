...
def latest_scan_context(name: str | None) -> dict | None:
    if not name:
        return None
    with conn() as c:
        r=c.execute("SELECT id,ts,image,status,critical,high,medium FROM scans WHERE container_name=? ORDER BY id DESC LIMIT 1",(name,)).fetchone()
...
async def security_score(authorization: str | None = Header(default=None)):
    ...
    for item in containers:
        ...
        if cr:
            ...
            # recurring behavior, and a recent clean Trivy scan. It never suppresses evidence.
            if src_list==['falco'] or set(src_list)=={'falco'}:
                with conn() as c:
            ...
            # Severity counters describe effective Kingdom risk after adaptive context.
            effective_risk='low' if cr_score<30 else ('medium' if cr_score<60 else ('high' if cr_score<90 else 'critical'))
            if effective_risk in severity_counts: severity_counts[effective_risk]+=1
            if cr_score > 0:
                srcs=', '.join(src_list) if isinstance(src_list,list) else 'security evidence'
                factors.append(f"Decision Engine: {effective_risk} ({cr_score} effective risk points; {srcs or 'single source'})")
...
async def baseline_suggestions(days: int = 7, authorization: str | None = Header(default=None)):
    ...
    for (name,rule),g in groups.items():
        ...
        scan=latest_scan_context(name)
        clean=bool(scan and scan.get('status')=='ok' and not int(scan.get('critical') or 0) and not int(scan.get('high') or 0) and not int(scan.get('medium') or 0))
        g.update(b); g.update({'suggested':b['status']=='stable','auto_applied':False,'clean_trivy':clean,'effective_score_adjustment':b['score_adjustment'] if clean else 0})
        out.append(g)
...
<div class="card containers"><h2 class="section-title">CONTAINER LIFE & CONTROLS</h2><div id="containers"></div></div><div class="footer"><span>🧠 Decision Engine <b class="good">Active</b></span><span>🛡 Versioned DB migrations</span><span id="footerEval">Last evaluation —</span><span>v1.11.0 · Adaptive Intelligence + Baseline-Aware Risk ♛</span></div></div>
<script>
