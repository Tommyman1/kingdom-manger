...
async def _probe_falco() -> dict:
    ...
    try:
        ...
        out.update({"status":"ok","detail":"Falco private health listener reachable",
                    "latency_ms":round((time.monotonic()-started)*1000),"source":"direct"})
    except Exception as e:
        if webhook_fresh:
            out.update({"status":"ok","detail":"Falco webhook traffic is fresh",
                        "latency_ms":round((time.monotonic()-started)*1000),"source":"webhook"})
        else:
            out.update({"status":"down","detail":f"{type(e).__name__}: {e}",
                        "latency_ms":round((time.monotonic()-started)*1000),"source":"none"})
    finally:
        if writer is not None:
