...
def is_suppressed(source: str, name: str | None, message: str) -> tuple[bool,str]:
    ...
    with conn() as c:
        rows=c.execute("SELECT container_name,rule_contains,reason FROM suppressions WHERE enabled=1 AND source=? AND (expires_ts IS NULL OR expires_ts>strftime('%s','now'))",(source.lower(),)).fetchall()
    for r in rows:
...
async def build_incident_investigation(incident_id: int, persist: bool = True) -> dict:
    ...
    trivy_clean=bool(scan and scan['status']=='ok' and int(scan['critical'] or 0)==0 and int(scan['high'] or 0)==0 and int(scan['medium'] or 0)==0)
    trivy_no_critical_high=bool(scan and scan['status']=='ok' and int(scan['critical'] or 0)==0 and int(scan['high'] or 0)==0)
    ...
    if classification=='likely-expected': intelligence+=f"No independent malware/network corroboration is active. Kingdom currently rates this pattern LIKELY EXPECTED at {confidence}% confidence."
    ...
    recommended_action='mark-expected' if classification=='likely-expected' and confidence>=80 else ('capture-and-isolate' if classification in {'suspicious','high-confidence-threat'} else 'continue-investigation')
    assessment={'incident_id':incident_id,'container':name,'classification':classification,'confidence':confidence,'confidence_delta':confidence_delta,'confidence_math':confidence_math,'summary':summary,'intelligence_summary':intelligence,'recommended_action':recommended_action,'factors':factors,'source_counts':source_counts,'strongest':strongest,'falco_rules':falco_rules[:10],'top_rule':top_rule,'matching_falco_events':matching_events,'falco_total_24h':int(falco_total_24h or 0),'first_observed':first_ts,'last_observed':last_ts,'latest_scan':dict(scan) if scan else None,'policy':policy,'risk_profile':rp,'suppressions':rowdicts(suppressions),'recovery_available':bool(name and not recovery_reasons),'recovery_reasons':recovery_reasons,'score_snapshot':score_snapshot,'score_delta':score_delta}
    if persist:
        details={'confidence_math':confidence_math,'matching_falco_events':matching_events,'falco_total_24h':int(falco_total_24h or 0),'recommended_action':recommended_action}
...
async def baseline_suggestions(days: int = 7, authorization: str | None = Header(default=None)):
    require_token(authorization); cutoff=now()-max(1,min(days,30))*86400
    with conn() as c:
        rows=c.execute("SELECT container_name,raw_json,message,ts FROM security_events WHERE source='falco' AND ts>? AND container_name IS NOT NULL ORDER BY ts",(cutoff,)).fetchall()
    groups={}
    for r in rows:
        raw=_safe_json(r['raw_json'],{}) or {}; rule=raw.get('rule') or falco_rule_from_message(r['message']) or 'Other Falco rule'; key=(r['container_name'],rule)
        g=groups.setdefault(key,{'container':r['container_name'],'rule':rule,'count':0,'first_ts':int(r['ts']),'last_ts':int(r['ts']),'days_seen':set()}); g['count']+=1; g['first_ts']=min(g['first_ts'],int(r['ts'])); g['last_ts']=max(g['last_ts'],int(r['ts'])); g['days_seen'].add(datetime.fromtimestamp(int(r['ts']),TZ).date().isoformat())
    out=[]
    for g in groups.values():
        if g['count']<100: continue
        span_hours=max(0,(g['last_ts']-g['first_ts'])/3600); days_seen=len(g.pop('days_seen'))
        confidence=min(95,50+min(25,g['count']//100*3)+min(20,days_seen*5))
        g.update({'span_hours':round(span_hours,1),'days_seen':days_seen,'baseline_confidence':confidence,'suggested':days_seen>=2 or span_hours>=12,'auto_applied':False})
        out.append(g)
    out.sort(key=lambda x:(x['suggested'],x['count']),reverse=True)
    return out[:50]
...
<div class="card containers"><h2 class="section-title">CONTAINER LIFE & CONTROLS</h2><div id="containers"></div></div><div class="footer"><span>🧠 Decision Engine <b class="good">Active</b></span><span>🛡 Versioned DB migrations</span><span id="footerEval">Last evaluation —</span><span>v1.10.0 · Intelligence + Explainability + Noise Control ♛</span></div></div>
<script>
