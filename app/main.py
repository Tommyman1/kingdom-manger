...
UPDATE_ALLOW_RISK_REDUCTION = os.getenv("KM_UPDATE_ALLOW_RISK_REDUCTION", "true").lower() in {"1","true","yes","on"}
...
def _docker_demux_exec_stream(raw: bytes) -> tuple[bytes, bytes]:
    """Decode Docker application/vnd.docker.raw-stream frames."""
    if not raw:
        return b"", b""
    if len(raw) < 8 or raw[0] not in (0, 1, 2, 3) or raw[1:4] != b"\x00\x00\x00":
        return raw, b""
    stdout, stderr = bytearray(), bytearray()
    i = 0
    while i + 8 <= len(raw):
        stream_type = raw[i]
        if raw[i+1:i+4] != b"\x00\x00\x00":
            stdout.extend(raw[i:])
            break
...
async def trivy_exec(cmd: list[str], timeout: int = 900) -> tuple[int, str]:
    """Run Trivy through its isolated exec proxy and demultiplex Docker output."""
    client = _http_client()
    try:
        cr = await client.post(
            TRIVY_EXEC_DOCKER + f"/containers/{quote(TRIVY_RUNNER, safe='')}/exec",
        ...
        exid = cr.json()["Id"]
        r = await client.post(
            TRIVY_EXEC_DOCKER + f"/exec/{exid}/start",
            json={"Detach": False, "Tty": False},
            timeout=timeout,
        )
        out_b, err_b = _docker_demux_exec_stream(r.content)
        ...
        text = stdout if int(exit_code or 0) == 0 else (stdout + ("\n" if stdout and stderr else "") + stderr)
        return int(exit_code or 0), text
    except httpx.HTTPError as e:
        raise HTTPException(503, f"Trivy execution proxy unavailable: {type(e).__name__}: {e}")


def _trivy_error_text(result_json: str | dict | None) -> str:
    raw = result_json
    if isinstance(raw, str):
        raw = _safe_json(raw, {}) or {}
    if isinstance(raw, dict):
        return str(raw.get("error") or raw.get("raw") or "Trivy scan failed")[-1200:]
    return "Trivy scan failed"



async def _trivy_scan_ref(image_ref: str) -> dict:
    counts={"critical":0,"high":0,"medium":0,"low":0,"unknown":0}
    if not image_ref:
        return {"status":"error","error":"missing image reference",**counts,"total":0,"findings":[]}
    findings=[]
    try:
        cmd=["trivy","image","--quiet","--format","json","--scanners","vuln","--image-src","docker,remote","--exit-code","0","--timeout","10m",image_ref]
        exit_code,raw=await trivy_exec(cmd,timeout=720)
        if exit_code!=0:
            return {"status":"error","image":image_ref,"error":f"Trivy exited with code {exit_code}: {raw[-4000:]}",**counts,"total":0,"findings":[]}
        p=raw.find("{")
        if p>0: raw=raw[p:]
        ...
        for section in result.get("Results") or []:
            ...
            for v in section.get("Vulnerabilities") or []:
                ...
                findings.append({
                    "target":target,"vuln_id":str(v.get("VulnerabilityID") or ""),
                    "pkg_name":str(v.get("PkgName") or ""),"installed_version":str(v.get("InstalledVersion") or ""),
                    "fixed_version":str(v.get("FixedVersion") or ""),"severity":sev,
                    "title":str(v.get("Title") or v.get("Description") or "")[:1000],
                })
        return {"status":"ok","image":image_ref,**counts,"total":len(findings),"findings":findings}
    except Exception as e:
        return {"status":"error","image":image_ref,"error":f"{type(e).__name__}: {e}",**counts,"total":0,"findings":[]}

def _vuln_key(f: dict) -> str:
    return f"{str(f.get('vuln_id') or '').strip()}|{str(f.get('pkg_name') or '').strip()}"

def _risk_points(scan: dict) -> int:
    return int(scan.get("critical",0) or 0)*10000 + int(scan.get("high",0) or 0)*100 + int(scan.get("medium",0) or 0)
...
async def trivy_scan(name: str) -> dict:
    ...
    with conn() as c:
        cur = c.execute(
            ...
            (started, name, image, "running", 0, 0, 0,
             json.dumps({"state": "running", "started_ts": started}))
        )
    ...
    try:
        cmd = [
            ...
            "--exit-code", "0", "--timeout", "10m", image,
        ]
        exit_code, raw = await trivy_exec(cmd, timeout=720)
        if exit_code != 0:
            result = {"error": f"Trivy exited with code {exit_code}", "raw": raw[-20000:]}
        else:
            p = raw.find("{")
            if p > 0:
                raw = raw[p:]
    ...
    with conn() as c:
        c.execute(
            "UPDATE scans SET ts=?,status=?,critical=?,high=?,medium=?,result_json=? WHERE id=?",
            (now(), status, counts["critical"], counts["high"], counts["medium"],
             json.dumps(result, default=str)[:1000000], scan_id)
        )
        if status == "ok":
            c.executemany(
                ...
                [(scan_id, name, image, f["target"], f["vuln_id"], f["pkg_name"],
                  f["installed_version"], f["fixed_version"], f["severity"], f["title"])
                 for f in findings]
            )
    ...
    if status == "ok" and (counts["critical"] or counts["high"]):
        ...
        if counts["critical"]:
            await notify(
                ...
                f"{name} ({image}) has {counts['critical']} critical and {counts['high']} high findings. No isolation occurs from Trivy alone.",
                "critical"
            )

    top = [f for f in findings if f["severity"] in {"critical", "high"}][:20]
    return {
        "scan_id": scan_id, "container": name, "image": image, "status": status,
        **counts, "total": len(findings), "top_findings": top, "decision": decision,
        "error": _trivy_error_text(result) if status != "ok" else None,
    }
...
def _sensor_heartbeat_set(sensor: str, ts: int | None = None, detail: dict | None = None):
    with conn() as c:
        c.execute("""CREATE TABLE IF NOT EXISTS sensor_heartbeats(
            sensor TEXT PRIMARY KEY,
            last_seen_ts INTEGER NOT NULL,
            detail_json TEXT NOT NULL DEFAULT '{}'
        )""")
        c.execute("""INSERT INTO sensor_heartbeats(sensor,last_seen_ts,detail_json)
                     VALUES(?,?,?)
                     ON CONFLICT(sensor) DO UPDATE SET
                       last_seen_ts=excluded.last_seen_ts,
                       detail_json=excluded.detail_json""",
                  (sensor,int(ts or now()),json.dumps(detail or {},default=str)))
        c.commit()

def _sensor_heartbeat_get(sensor: str) -> dict:
    with conn() as c:
        c.execute("""CREATE TABLE IF NOT EXISTS sensor_heartbeats(
            sensor TEXT PRIMARY KEY,
            last_seen_ts INTEGER NOT NULL,
        ...
        row=c.execute("SELECT last_seen_ts,detail_json FROM sensor_heartbeats WHERE sensor=?",(sensor,)).fetchone()
    if not row:
        return {"sensor":sensor,"last_seen_ts":None,"detail":{}}
    try:
        detail=json.loads(row["detail_json"] or "{}")
    except Exception:
        detail={}
    return {"sensor":sensor,"last_seen_ts":int(row["last_seen_ts"]),"detail":detail}
...
async def falco_webhook(request: Request, token: str = ""):
    ...
    with conn() as c:
        c.execute("INSERT INTO security_events(ts,source,severity,container_name,message,raw_json) VALUES(?,?,?,?,?,?)",
                  (received_ts, "falco", severity, name, msg, json.dumps(data, default=str)[:100000]))
        c.commit()
    _sensor_heartbeat_set("falco", received_ts, {
        "rule": data.get("rule"),
...
async def _probe_falco() -> dict:
    started=time.monotonic()
    hb=_sensor_heartbeat_get("falco")
    last_seen=hb.get("last_seen_ts")
    age_seconds=(now()-last_seen) if last_seen else None

    # Quiet runtime activity is normal. DOWN requires both a failed direct probe
    # and stale webhook evidence.
    webhook_fresh=bool(last_seen and age_seconds <= max(120,SENSOR_FAILURE_GRACE_SECONDS))
    webhook_recent=bool(last_seen and age_seconds <= max(3600,SENSOR_FAILURE_GRACE_SECONDS*5))

    out={"configured":True,"endpoint":f"{FALCO_HOST}:{FALCO_HEALTH_PORT}",
         "last_seen_ts":last_seen,"last_seen_age_seconds":age_seconds,
         "webhook_fresh":webhook_fresh,"webhook_recent":webhook_recent}
    writer=None
    try:
        reader,writer=await asyncio.wait_for(asyncio.open_connection(FALCO_HOST,FALCO_HEALTH_PORT),timeout=2)
        out.update({"status":"ok","detail":"Falco private health listener reachable",
                    "latency_ms":round((time.monotonic()-started)*1000),
                    "source":"direct","direct_health_ok":True})
    except Exception as e:
        if webhook_recent:
            out.update({"status":"quiet",
                        "detail":"Falco webhook delivery was seen recently; direct health listener is unavailable",
                        "latency_ms":round((time.monotonic()-started)*1000),
                        "source":"webhook","direct_health_ok":False,
                        "probe_error":f"{type(e).__name__}: {e}"})
        else:
            out.update({"status":"down","detail":f"{type(e).__name__}: {e}",
                        "latency_ms":round((time.monotonic()-started)*1000),
                        "source":"none","direct_health_ok":False})
    finally:
        if writer is not None:
...
async def falco_summary(authorization: str | None = Header(default=None)):
    ...
    with conn() as c:
        ...
        effective = c.execute("""SELECT severity,count(*) n FROM incidents
                                WHERE status NOT IN ('resolved','dismissed')
                                  AND lower(sources_json) LIKE '%falco%'
                                GROUP BY severity""").fetchall()
        known_good = c.execute("""SELECT count(DISTINCT se.id) n
                                  FROM security_events se
                                  JOIN suppressions s ON s.source='falco' AND s.enabled=1
                                   AND (s.container_name IS NULL OR s.container_name=se.container_name)
                                   AND (s.expires_ts IS NULL OR s.expires_ts>?)
                                  WHERE se.source='falco' AND se.ts>?
                                   AND (
                                     lower(coalesce(se.message,'')) LIKE '%'||lower(coalesce(s.rule_contains,''))||'%'
                                     OR lower(coalesce(se.raw_json,'')) LIKE '%'||lower(coalesce(s.rule_contains,''))||'%'
                                   )""", (now(), cutoff)).fetchone()
    recent=[]
    for r in rows:
    ...
    return {
        "last_webhook_ts": hb_ts,
        "last_webhook_age_seconds": hb_age,
        "webhook_connected": bool(hb_ts and hb_age <= max(120,SENSOR_FAILURE_GRACE_SECONDS)),
        "events_24h": sum(r["n"] for r in counts),
        "counts_24h": {r["severity"]: r["n"] for r in counts},
        "raw_counts_24h": {r["severity"]: r["n"] for r in counts},
        "effective_incidents": {r["severity"]: r["n"] for r in effective},
        "known_good_events_24h": int(known_good["n"] or 0) if known_good else 0,
        "last_event_ts": latest["ts"] if latest else None,
        "last_event_age_seconds": max(0, now()-latest["ts"]) if latest else None,
...
async def trivy_summary(authorization: str | None = Header(default=None)):
    ...
    with conn() as c:
        ...
        counts = c.execute("SELECT count(*) n,coalesce(sum(critical),0) critical,coalesce(sum(high),0) high,coalesce(sum(medium),0) medium FROM scans WHERE ts>? AND status='ok'", (start,)).fetchone()
        states = c.execute("SELECT status,count(*) n FROM scans WHERE ts>? GROUP BY status", (start,)).fetchall()
    ...
    state_counts={r["status"]:r["n"] for r in states}
    return {
        "scans_24h": counts["n"], "successful_scans_24h": counts["n"],
        "failed_scans_24h": state_counts.get("error",0), "running_scans": state_counts.get("running",0),
        "critical_24h": counts["critical"], "high_24h": counts["high"], "medium_24h": counts["medium"],
        "recent_scans": rowdicts(scans), "top_findings": rowdicts(findings),
...
def _local_build_guidance(name: str, image: str, scan: dict | None=None) -> dict:
    scan=scan or {}
    critical=int(scan.get('critical') or 0); high=int(scan.get('high') or 0); medium=int(scan.get('medium') or 0)
    return {
        'container':name,'image':image,'local_build':True,'registry_update_supported':False,
        'critical':critical,'high':high,'medium':medium,
        'message':(
            'Local image: registry pull/update is disabled. To address vulnerabilities, update the project Dockerfile base image and/or dependencies, rebuild from source, recreate the container, then rescan with Trivy. Rebuilding unchanged source may reproduce the same vulnerabilities.'
            if critical or high or medium else
            'Local image: registry pull/update is disabled. Rebuild only when you intentionally change the project source, base image, or dependencies.'
        )
    }
...
async def update_check(name: str, authorization: str|None=Header(default=None)):
    require_token(authorization)
    obj=await inspect_container(name); image=(obj.get('Config') or {}).get('Image'); old_id=obj.get('Image')
    if not image: raise HTTPException(409,'Container has no image reference')
    policy=default_policy(name)
    if bool(policy.get('local_build')):
        with conn() as c:
            last=c.execute("SELECT ts,critical,high,medium,status FROM scans WHERE container_name=? AND status='ok' ORDER BY id DESC LIMIT 1",(name,)).fetchone()
        guidance=_local_build_guidance(name,image,dict(last) if last else None)
        audit('update-check',name,'local-build',guidance)
        return {'status':'local-build','container':name,'image':image,'guidance':guidance}
    # Registry-backed image: capture immutable rollback state before pulling candidate.
...
async def update_verify(plan_id: int, authorization: str|None=Header(default=None)):
    ...
    if p['status']=='current': return {'ok':True,'status':'current','plan_id':plan_id,'operator_approval_required':False}
    if not UPDATE_REQUIRE_TRIVY:
        status='verified'; detail={'status':'skipped','comparison':None}
    else:
        current=await _trivy_scan_ref(str(p['old_image_id'] or p['image_ref']))
        candidate=await _trivy_scan_ref(str(p['candidate_image_id'] or p['image_ref']))
        if current.get('status')!='ok' or candidate.get('status')!='ok':
            status='blocked'; detail={'status':'error','current':current,'candidate':candidate,'comparison':None}
        else:
            comparison=_compare_scans(current,candidate)
            hard_block=((UPDATE_BLOCK_CRITICAL and int(candidate.get('critical',0) or 0)>0) or (UPDATE_BLOCK_HIGH and int(candidate.get('high',0) or 0)>0))
            status='verified' if not hard_block else ('risk-reduction' if UPDATE_ALLOW_RISK_REDUCTION and comparison.get('safer') else 'blocked')
            detail={'status':'ok','current':{k:v for k,v in current.items() if k!='findings'},'candidate':{k:v for k,v in candidate.items() if k!='findings'},'comparison':{**{k:v for k,v in comparison.items() if not k.endswith('_findings')},'fixed_findings':comparison['fixed_findings'][:100],'remaining_findings':comparison['remaining_findings'][:100],'introduced_findings':comparison['introduced_findings'][:100]}}
    with conn() as c: c.execute('UPDATE update_plans SET status=?,scan_json=? WHERE id=?',(status,json.dumps(detail,default=str)[:1000000],plan_id))
    audit('update-verify',p['container_name'],status,{'plan_id':plan_id,'verification':detail})
    return {'ok':status=='verified','status':status,'plan_id':plan_id,'operator_approval_required':status=='risk-reduction','verification':detail,'trivy':detail.get('candidate') if isinstance(detail,dict) else None}

@app.post('/api/updates/{plan_id}/approve-risk-reduction')
async def approve_risk_reduction(plan_id: int, authorization: str|None=Header(default=None)):
    require_token(authorization)
    with conn() as c:
        p=c.execute('SELECT * FROM update_plans WHERE id=?',(plan_id,)).fetchone()
        if not p: raise HTTPException(404,'Update plan not found')
        if p['status']!='risk-reduction': raise HTTPException(409,'Plan is not awaiting risk-reduction approval')
        detail=_safe_json(p['scan_json'],{}) or {}; comp=detail.get('comparison') or {}
        if not comp.get('safer'): raise HTTPException(409,'Candidate is not verified as safer than the running image')
        c.execute("UPDATE update_plans SET status='verified-risk-reduction',approved_ts=? WHERE id=?",(now(),plan_id))
    audit('update-risk-approval',p['container_name'],'approved',{'plan_id':plan_id,'comparison':comp})
    return {'ok':True,'status':'verified-risk-reduction','plan_id':plan_id,'comparison':comp}
        ...
        try:
            if UPDATE_ENGINE_ENABLED and not global_maintenance_active():
                ...
                for x in cs:
                    ...
                    if pol.get('auto_update') and not pol.get('protected') and not pol.get('local_build'): eligible.append((int(pol.get('update_ring') or 2),x['name']))
                for ring,name in sorted(eligible):
                    try:
                        ...
                        if plan.get('status')=='available':
                            ...
                            elif ver.get('status')=='risk-reduction':
                                event('update_engine',name,{'state':'awaiting-operator-risk-approval','ring':ring,'plan_id':plan.get('plan_id')},'warning')
...
@app.get("/api/containers/{name}/vulnerabilities")
async def container_vulnerabilities(name: str, authorization: str|None=Header(default=None)):
    require_token(authorization)
    with conn() as c:
        scan=c.execute("SELECT id,ts,image,status,critical,high,medium FROM scans WHERE container_name=? AND status='ok' ORDER BY id DESC LIMIT 1",(name,)).fetchone()
        if not scan: return {'container':name,'scan':None,'findings':[],'fixable_count':0,'unfixed_count':0}
        findings=rowdicts(c.execute("SELECT vuln_id,pkg_name,installed_version,fixed_version,severity,target,title FROM scan_findings WHERE scan_id=? ORDER BY CASE severity WHEN 'critical' THEN 0 WHEN 'high' THEN 1 WHEN 'medium' THEN 2 WHEN 'low' THEN 3 ELSE 4 END,pkg_name,vuln_id",(scan['id'],)).fetchall())
    for f in findings:
        f['fix_available']=bool(str(f.get('fixed_version') or '').strip())
...
async def remediation_plan(name: str, authorization: str|None=Header(default=None)):
    require_token(authorization)
    policy=default_policy(name); obj=await inspect_container(name)
    image=(obj.get('Config') or {}).get('Image') or ''; current_id=obj.get('Image') or image
    current=await _trivy_scan_ref(current_id)
    if current.get('status')!='ok': raise HTTPException(503,{'message':'Unable to scan current image','scan':current})
    ...
    check=await update_check(name,authorization)
    if check.get('status')=='current':
        return {'container':name,'mode':'registry','status':'no-new-image','current':{k:v for k,v in current.items() if k!='findings'},'fixable_count':0,'remaining_count':len(current.get('findings') or []),'stateful_gate':stateful_gate,'findings':current.get('findings') or []}
    plan_id=int(check['plan_id']); verify=await update_verify(plan_id,authorization)
    return {'container':name,'mode':'registry','status':verify.get('status'),'plan_id':plan_id,'rollback_snapshot':check.get('rollback_snapshot'),'verification':verify.get('verification'),'stateful_gate':stateful_gate,'operator_approval_required':verify.get('operator_approval_required',False),'can_apply_now':bool(verify.get('ok')),'can_apply_safer_candidate':bool(verify.get('operator_approval_required'))}
...
async def recommendations(authorization: str | None = Header(default=None)):
    ...
    with conn() as c:
        for item in cs:
            ...
            attempt=c.execute("SELECT ts,critical,high,medium,status,result_json FROM scans WHERE container_name=? ORDER BY id DESC LIMIT 1",(name,)).fetchone()
            success=c.execute("SELECT ts,critical,high,medium,status FROM scans WHERE container_name=? AND status='ok' ORDER BY id DESC LIMIT 1",(name,)).fetchone()
            if attempt and attempt['status']=='running':
                if t-int(attempt['ts']) > 1200:
                    rec.append({'priority':'medium','title':f'Retry stale Trivy scan for {name}','detail':'Trivy state: STALE. The prior scan has been RUNNING for more than 20 minutes and is not treated as clean evidence.','action':'scan','container':name,'scan_state':'STALE'})
            elif attempt and attempt['status']!='ok':
                reason=_trivy_error_text(attempt['result_json']).replace("\n"," ")[:180]
                last_ok=(f" Last successful: {success['critical']} critical · {success['high']} high · {success['medium']} medium, {max(0,(t-int(success['ts']))//60)}m ago." if success else " No successful scan exists yet.")
                rec.append({'priority':'medium','title':f'Retry Trivy scan for {name}','detail':f'Latest state: ERROR. Reason: {reason}.{last_ok}','action':'scan','container':name,'scan_state':'ERROR'})
            elif not success:
                rec.append({'priority':'medium','title':f'Scan {name}','detail':'Trivy state: NEVER_SCANNED. No successful vulnerability result exists for this running container yet.','action':'scan','container':name,'scan_state':'NEVER_SCANNED'})
...
/* v3.1.6 profile navigation */
.km-profile-action{min-height:40px;margin-top:10px;padding:8px 14px;display:inline-flex;align-items:center;justify-content:center;gap:7px;white-space:nowrap}

/* v3.1.7 per-container remediation command center */
.remediation-panel{border:1px solid #294b60;background:linear-gradient(180deg,rgba(20,53,70,.32),rgba(7,22,31,.2))}
.remediation-head{display:flex;justify-content:space-between;gap:14px;align-items:flex-start;margin-bottom:10px}
.remediation-head h3{margin:0 0 4px}
.remediation-item{display:flex;align-items:center;justify-content:space-between;gap:16px;padding:12px 0;border-top:1px solid #173244}
.remediation-item strong{display:inline-block;margin-left:8px}
.remediation-item .tiny{margin-top:5px;max-width:620px}
.remediation-action{flex:0 0 auto}
.remediation-clear{padding:13px;border:1px solid #17663b;border-radius:10px;color:#54e88b;background:rgba(16,95,52,.14)}
@media(max-width:760px){.remediation-item{align-items:stretch;flex-direction:column}.remediation-action button{width:100%}}


/* v3.1.9 container list profile navigation */
.container-name-link{
  appearance:none;background:none;border:0;padding:0;margin:0 4px 0 0;
  color:#eef7ff;font:inherit;font-weight:800;cursor:pointer;
}
.container-name-link:hover .container-name-text{color:#39bfff;text-decoration:underline;text-underline-offset:3px}
.container-name-link:focus-visible{outline:2px solid #39bfff;outline-offset:3px;border-radius:5px}
.btn-profile{
  border:1px solid #1d8fc7;background:rgba(29,143,199,.14);color:#7fd7ff;
  min-height:36px;padding:7px 12px;border-radius:9px;font-weight:700;
}
.btn-profile:hover{background:rgba(29,143,199,.24);border-color:#39bfff;color:#b8ecff}
.container-actions{display:flex;align-items:center;justify-content:flex-end;gap:8px;flex-wrap:wrap}
.container-actions button:disabled{opacity:.42;cursor:not-allowed}
@media(max-width:900px){
  .container-row{align-items:flex-start}
  .container-actions{justify-content:flex-start;margin-top:10px}
}
...
/* v3.2.6 structured modal compatibility */
#kmModal .km-modal-wide,
#kmModal .modal-card.km-modal-wide{
  width:min(880px,calc(100vw - 32px));
  max-width:880px;
}
#kmModalText.km-dialog-html{
  white-space:normal;
  text-align:left;
  max-height:68vh;
  overflow:auto;
...
@media(max-width:720px){
  ...
  #kmModalText.km-dialog-html{max-height:72vh}
}
...
let TOKEN=localStorage.getItem('km_token')||'',ACTIVITY_FILTER='',RECS=[];const API_GETS=new Map(),ACTIONS=new Set();function hdr(){return {'Authorization':'Bearer '+TOKEN,'Content-Type':'application/json'}}async function _api(url,opt={}){let ctrl=new AbortController(),timeout=Number(opt.timeout||20000),timer=setTimeout(()=>ctrl.abort(),timeout);delete opt.timeout;opt.headers={...(opt.headers||{}),...hdr()};opt.signal=opt.signal||ctrl.signal;try{let r=await fetch(url,opt);let type=(r.headers.get('content-type')||'').toLowerCase(),text=await r.text();if(r.status===401){document.getElementById('login').classList.remove('hidden');throw Error('Invalid or expired Kingdom Manager token')}if(!type.includes('application/json')){throw Error(`Kingdom API returned ${r.status} ${r.statusText||''} instead of JSON. The proxy or backend may be temporarily unavailable.`)}let d={};try{d=text?JSON.parse(text):{}}catch{throw Error(`Kingdom API returned malformed JSON (${r.status})`)}if(!r.ok)throw Error(typeof d.detail==='string'?d.detail:(d.detail?JSON.stringify(d.detail):`Request failed (${r.status})`));return d}catch(e){if(e.name==='AbortError')throw Error(`Kingdom request timed out after ${Math.round(timeout/1000)}s: ${url}`);throw e}finally{clearTimeout(timer)}}async function api(url,opt={}){let method=(opt.method||'GET').toUpperCase();if(method==='GET'){let key=url;if(API_GETS.has(key))return API_GETS.get(key);let p=_api(url,opt).finally(()=>API_GETS.delete(key));API_GETS.set(key,p);return p}return _api(url,opt)}async function guarded(key,label,fn){if(ACTIONS.has(key)){toast(`${label} is already running`,'warn');return null}ACTIONS.add(key);let btn=(typeof event!=='undefined'&&event?.currentTarget instanceof HTMLElement)?event.currentTarget:null,old=btn?.textContent;if(btn){btn.disabled=true;btn.classList.add('action-busy');btn.textContent=label+'…'}try{return await fn()}finally{ACTIONS.delete(key);if(btn){btn.disabled=false;btn.classList.remove('action-busy');btn.textContent=old}}}async function saveToken(btn=null){let err=document.getElementById('loginerr');TOKEN=document.getElementById('token').value.trim();err.textContent='';if(!TOKEN){err.textContent='Enter the Kingdom Manager token';return}if(btn){btn.disabled=true;btn.textContent='Verifying…'}try{let verified=false,lastErr=null;for(let attempt=0;attempt<2&&!verified;attempt++){try{await _api('/api/auth/verify',{timeout:15000});verified=true}catch(e){lastErr=e;if(attempt===0&&(/timed out|502|503|504|temporarily unavailable/i.test(e.message))){err.textContent='Kingdom is waking up — retrying connection…';await new Promise(r=>setTimeout(r,750));continue}throw e}}if(!verified)throw lastErr||Error('Unable to verify Kingdom token');localStorage.setItem('km_token',TOKEN);document.getElementById('login').classList.add('hidden');toast('Kingdom unlocked','ok');try{let boot=await _api('/api/dashboard/bootstrap',{timeout:3000});if(boot?.score)renderWarmScore(boot.score,boot.saved_ts)}catch{}load().catch(e=>toast(e.message,'error'))}catch(e){localStorage.removeItem('km_token');err.textContent=e.message}finally{if(btn){btn.disabled=false;btn.textContent='Unlock Dashboard'}}}function logout(){localStorage.removeItem('km_token');TOKEN='';document.getElementById('login').classList.remove('hidden')}function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}function age(sec){if(sec==null)return'never';if(sec<60)return sec+'s ago';if(sec<3600)return Math.floor(sec/60)+'m ago';return Math.floor(sec/3600)+'h ago'}function toast(msg,tone='ok'){let box=document.getElementById('toastbox'),el=document.createElement('div');el.className='toast '+(tone==='error'?'error':tone==='warn'?'warn':'');el.textContent=msg;box.appendChild(el);setTimeout(()=>el.remove(),5000)}function kmDialog({title='Kingdom Manager',text='',html='',input=false,value='',ok='Continue',cancel='Cancel',danger=false,wide=false}={}){
  return new Promise(resolve=>{
    ...
    // Preserve the existing modal DOM and input behavior, but allow the
    // remediation center to render trusted dashboard-generated structured HTML.
    if(html){
      textEl.innerHTML=html;
      textEl.classList.add('km-dialog-html');
    }else{
      textEl.textContent=text;
      textEl.classList.remove('km-dialog-html');
    }

    if(card)card.classList.toggle('km-modal-wide',!!wide);

    i.classList.toggle('hidden',!input);
    i.value=value;
    m.classList.remove('hidden');

    if(input)setTimeout(()=>i.focus(),30);
    ...
    let done=v=>{
      ...
      if(card)card.classList.remove('km-modal-wide');
      textEl.classList.remove('km-dialog-html');
      b.onclick=c.onclick=null;
...
async function fixWhatKingdomCan(n){return guarded('remediate:'+n,'Planning fixes',async()=>{try{
  toast(`Comparing ${n} against the newest available image…`,'warn');
  let r=await api(`/api/containers/${encodeURIComponent(n)}/remediation/plan`,{method:'POST',timeout:1500000});
  if(r.mode==='local-build'){
    let ok=await kmDialog({title:`Local remediation · ${n}`,text:`Kingdom found ${r.fixable_count||0} findings with listed fixed versions. This is a Local Build, so the Dockerfile/base image/dependencies must be changed and rebuilt before the fixes can persist.`,ok:'View Issues'});
    if(ok)await showVulnerabilities(n);return
  }
  if(r.status==='no-new-image'){
    await kmDialog({title:`No newer fix image · ${n}`,text:`No newer registry image is available. The running image still has ${r.current?.critical||0} critical · ${r.current?.high||0} high · ${r.current?.medium||0} medium findings. Kingdom will not patch files inside a third-party image because those changes disappear on recreation.`,ok:'View Issues'});
...
async function showLocalBuildHelp(n){try{let d=await api(`/api/containers/${encodeURIComponent(n)}/security-profile`),sc=d.last_successful_scan||{};let text=`${n} is marked LOCAL BUILD.

Kingdom will not pull this image from Docker Hub/GHCR.

Vulnerability fix path:
1. Update the project's Dockerfile base image and/or dependencies.
2. Rebuild the image from the updated local source.
3. Recreate the container using its Compose configuration.
4. Run Trivy again.
5. Keep rollback state until health verification passes.

Last successful scan: ${sc.critical||0} critical · ${sc.high||0} high · ${sc.medium||0} medium.
...
async function checkUpdate(n){return fixWhatKingdomCan(n)}
...
<div class="toolbar container-actions" onclick="event.stopPropagation()">
  <button class="btn-profile btn-compact" onclick="openDrawer('${esc(c.name)}')">🛡 Profile</button>
  ${c.state==='running'
    ? `<button class="btn-secondary btn-compact" disabled title="Container is already running">Start</button>`
    : `<button class="btn-secondary btn-compact" onclick="act('${esc(c.name)}','start')">Start</button>`}
  <button class="btn-secondary btn-compact" onclick="act('${esc(c.name)}','restart')">Restart</button>
  <button class="danger btn-compact" onclick="act('${esc(c.name)}','stop')">Stop</button>
  <button class="btn-secondary btn-compact" onclick="scan('${esc(c.name)}')">Trivy</button>
  ${p.local_build?`<button class="btn-local btn-compact" onclick="showLocalBuildHelp('${esc(c.name)}')" title="Local image: rebuild from source after updating dependencies">⌂ Local</button>`:`<button class="btn-secondary btn-compact" onclick="checkUpdate('${esc(c.name)}')">Update</button>`}
</div>
</div>`}).join('');document.getElementById('footerEval').textContent='Last full evaluation '+new Date(ss.evaluated_ts*1000).toLocaleTimeString()}
