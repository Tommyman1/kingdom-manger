...
async def build_incident_investigation(incident_id: int, persist: bool = True) -> dict:
    ...
    with conn() as c:
        ...
        scan=c.execute("SELECT id,ts,image,status,critical,high,medium FROM scans WHERE container_name=? ORDER BY id DESC LIMIT 1",(name,)).fetchone() if name else None
        suppressions=c.execute("SELECT source,container_name,rule_contains,reason FROM suppressions WHERE enabled=1 AND (container_name=? OR container_name IS NULL OR container_name='')",(name or '',)).fetchall()
    ...
    for r in sec:
        ...
        if src=='falco':
            ...
            if item: item['count']+=1; item['last_ts']=max(item['last_ts'],int(r['ts'])); item['first_ts']=min(item['first_ts'],int(r['ts']))
            else: falco_rules.append({'rule':rule,'count':1,'first_ts':int(r['ts']),'last_ts':int(r['ts']),'severity':sev})
    ...
    trivy_clean=bool(scan and scan['status']=='ok' and int(scan['critical'] or 0)==0 and int(scan['high'] or 0)==0)
    trivy_risky=bool(scan and (int(scan['critical'] or 0)>0 or int(scan['high'] or 0)>0))
    malware=bool('clamav' in strongest and strongest['clamav']['severity'] in {'critical','high'})
    multi=len(active_sources)>=2
    if malware:
        classification,confidence='high-confidence-threat',96; factors.append('ClamAV supplied high-severity malware evidence')
    elif multi and ('falco' in active_sources) and ('crowdsec' in active_sources or 'clamav' in active_sources):
        classification,confidence='suspicious',88; factors.append('Independent runtime and security-engine evidence corroborate each other')
    elif multi or trivy_risky:
        classification,confidence='suspicious',78; factors.append('Multiple evidence sources or vulnerable image context require review')
    elif 'falco' in active_sources and trivy_clean:
        classification,confidence='likely-expected',84; factors.append('Falco-only behavior with a recent Trivy scan showing no critical/high CVEs')
    elif 'falco' in active_sources and source_counts.get('falco',0)>=25:
        classification,confidence='likely-expected',72; factors.append('High-volume repeated Falco-only behavior without corroboration')
    elif active_sources:
    ...
    if scan: factors.append(f"Latest Trivy: {scan['critical']} critical / {scan['high']} high / {scan['medium']} medium")
    summary={
    ...
    assessment={'incident_id':incident_id,'container':name,'classification':classification,'confidence':confidence,'summary':summary,'factors':factors,'source_counts':source_counts,'strongest':strongest,'falco_rules':falco_rules[:10],'top_rule':top_rule,'first_observed':first_ts,'last_observed':last_ts,'latest_scan':dict(scan) if scan else None,'policy':default_policy(name) if name else None,'risk_profile':risk_profile(name) if name else None,'suppressions':rowdicts(suppressions)}
    if persist:
        with conn() as c: c.execute("INSERT INTO incident_assessments(incident_id,ts,classification,confidence,summary,factors_json,top_rule) VALUES(?,?,?,?,?,?,?)",(incident_id,t,classification,confidence,summary,json.dumps(factors),top_rule))
        event('investigation',name or 'host',{'incident_id':incident_id,'classification':classification,'confidence':confidence,'top_rule':top_rule})
    return assessment
...
async def incident_mark_expected(incident_id: int, request: Request, authorization: str | None = Header(default=None)):
    require_token(authorization); d=await request.json(); reason=str(d.get('reason','Known-good behavior approved by operator'))
    ...
    with conn() as c:
        c.execute("INSERT INTO suppressions(enabled,source,container_name,rule_contains,reason) VALUES(1,'falco',?,?,?) ON CONFLICT(source,container_name,rule_contains) DO UPDATE SET enabled=1,reason=excluded.reason",(name,rule,reason))
    ...
    event('suppression',name,{'incident_id':incident_id,'rule':rule,'reason':reason})
    return {'ok':True,'incident_id':incident_id,'container':name,'rule':rule,'status':'dismissed'}
...
async def activity_feed(limit: int = 50, include_idle: bool = False, kind: str = "", authorization: str | None = Header(default=None)):
    require_token(authorization)
...
<div class="card containers"><h2 class="section-title">CONTAINER LIFE & CONTROLS</h2><div id="containers"></div></div><div class="footer"><span>🧠 Decision Engine <b class="good">Active</b></span><span>🛡 Versioned DB migrations</span><span id="footerEval">Last evaluation —</span><span>v1.9.0 · Intelligence + Controlled Recovery ♛</span></div></div>
<script>
let TOKEN=localStorage.getItem('km_token')||'';function hdr(){return {'Authorization':'Bearer '+TOKEN,'Content-Type':'application/json'}}async function api(url,opt={}){opt.headers={...(opt.headers||{}),...hdr()};let r=await fetch(url,opt);if(r.status===401){document.getElementById('login').classList.remove('hidden');throw Error('Unauthorized')}let t=await r.text(),d=t?JSON.parse(t):{};if(!r.ok)throw Error(d.detail||t);return d}function saveToken(){TOKEN=document.getElementById('token').value.trim();localStorage.setItem('km_token',TOKEN);load().then(()=>document.getElementById('login').classList.add('hidden')).catch(e=>document.getElementById('loginerr').textContent=e.message)}function logout(){localStorage.removeItem('km_token');TOKEN='';document.getElementById('login').classList.remove('hidden')}function esc(s){return String(s??'').replace(/[&<>"']/g,m=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[m]))}function age(sec){if(sec==null)return'never';if(sec<60)return sec+'s ago';if(sec<3600)return Math.floor(sec/60)+'m ago';return Math.floor(sec/3600)+'h ago'}function toast(msg,tone='ok'){let box=document.getElementById('toastbox'),el=document.createElement('div');el.className='toast '+(tone==='error'?'error':tone==='warn'?'warn':'');el.textContent=msg;box.appendChild(el);setTimeout(()=>el.remove(),5000)}async function act(n,a){if((a==='stop'||a==='isolate')&&!confirm(`${a.toUpperCase()} ${n}?`))return;try{let url=a==='isolate'?`/api/containers/${encodeURIComponent(n)}/isolate`:`/api/containers/${encodeURIComponent(n)}/action/${a}`;await api(url,{method:'POST'});toast(`${a} completed for ${n}`);load()}catch(e){toast(e.message,'error')}}async function scan(n){try{let d=await api(`/api/containers/${encodeURIComponent(n)}/trivy`,{method:'POST'});toast(`Trivy ${n}: ${d.critical||0} critical · ${d.high||0} high · ${d.medium||0} medium`);load();if(!document.getElementById('drawer').classList.contains('hidden'))openDrawer(n)}catch(e){toast(e.message,'error')}}async function scanIncident(id,n){try{toast(`Trivy scan started for ${n}`,'warn');let d=await api(`/api/containers/${encodeURIComponent(n)}/trivy`,{method:'POST'});toast(`Trivy ${n}: ${d.critical||0} critical · ${d.high||0} high · ${d.medium||0} medium`);await load();await openIncident(id)}catch(e){toast(e.message,'error')}}async function upd(n){try{let d=await api(`/api/containers/${encodeURIComponent(n)}/check-update`,{method:'POST'});alert(d.update_available?'New image pulled; container was not recreated automatically.':'No image ID change detected.');load()}catch(e){alert(e.message)}}async function policy(n,key,value){try{await api(`/api/policies/${encodeURIComponent(n)}`,{method:'PUT',body:JSON.stringify({[key]:value})});load();if(!document.getElementById('drawer').classList.contains('hidden'))openDrawer(n)}catch(e){alert(e.message)}}function policyButton(n,label,key,on,extra=''){return `<button class="policybtn ${on?'on':'off'} ${extra}" onclick="event.stopPropagation();policy('${esc(n)}','${key}',${on?'false':'true'})">${on?'✓':'○'} ${label}</button>`}function engineCard(name,icon,status,body){let cls=status==='ok'?'good':status==='down'||status==='error'?'bad':'warn';return `<div class="card"><div class="engine-head"><span>${icon} ${name.toUpperCase()}</span><span class="tag ${cls}">${esc(status||'ready').toUpperCase()}</span></div>${body}<div class="spark"></div></div>`}
async function incidentStatus(id,status){await api(`/api/incidents/${id}/status`,{method:'POST',body:JSON.stringify({status})});load()}async function createRecovery(n,incidentId){try{let p=await api(`/api/recovery/plan/${encodeURIComponent(n)}`,{method:'POST',body:JSON.stringify({incident_id:incidentId})});if(!confirm(`Recovery plan #${p.plan_id} created for ${n}. This will capture evidence, quarantine the replacement, verify it with Trivy, then recreate and observe health. Approve and run now?`))return;let r=await api(`/api/recovery/${p.plan_id}/approve-and-run`,{method:'POST'});alert(r.ok?'Recovery completed successfully.':`Recovery stopped safely: ${r.reason||'review required'}`);load();if(incidentId)openIncident(incidentId);else openDrawer(n)}catch(e){alert(e.message)}}async function markIncidentExpected(id,rule){let reason=prompt('Why is this expected?','Known-good behavior for this container');if(!reason)return;await api(`/api/incidents/${id}/mark-expected`,{method:'POST',body:JSON.stringify({rule,reason})});closeDrawer();load()}async function isolateIncident(id){if(!confirm('Isolate this incident container from its current networks?'))return;try{await api(`/api/incidents/${id}/isolate`,{method:'POST'});openIncident(id);load()}catch(e){alert(e.message)}}async function openIncident(id){try{await incidentStatus(id,'investigating');let [inc,a]=await Promise.all([api(`/api/incidents/${id}`),api(`/api/incidents/${id}/investigation`)]);let n=inc.container_name||'',cls=a.classification||'unverified',tone=cls==='high-confidence-threat'?'bad':cls==='suspicious'?'warn':cls==='likely-expected'?'good':'';document.getElementById('drawerTitle').textContent=`Incident #${id} · ${n||'host'}`;document.getElementById('drawerSubtitle').textContent=`${inc.severity.toUpperCase()} · ${inc.status} · Kingdom assessment ${a.confidence}%`;let rules=(a.falco_rules||[]).map(x=>`<div class="event"><div><strong>${esc(x.rule)}</strong><span class="tiny">${x.count} events · first ${new Date(x.first_ts*1000).toLocaleString()} · last ${new Date(x.last_ts*1000).toLocaleString()}</span></div><span class="tag ${x.severity==='critical'?'bad':'warn'}">${esc(x.severity)}</span></div>`).join('')||'<div class="tiny">No Falco rules in the last 24h.</div>';let factors=(a.factors||[]).map(x=>`<div class="tiny" style="padding:5px 0">• ${esc(x)}</div>`).join('');let scan=a.latest_scan?`${a.latest_scan.critical} critical · ${a.latest_scan.high} high · ${a.latest_scan.medium} medium`:'Not scanned';let allow=a.policy?.allow_rebuild&&!a.policy?.protected;let top=a.top_rule||'';document.getElementById('drawerBody').innerHTML=`<section><h3>KINGDOM ASSESSMENT</h3><div class="kv"><b>Classification</b><span class="${tone}">${esc(cls).replaceAll('-',' ').toUpperCase()}</span></div><div class="kv"><b>Confidence</b><span>${a.confidence}%</span></div><p class="muted">${esc(a.summary)}</p>${factors}</section><section><h3>CORROBORATION</h3><div class="kv"><b>Falco</b><span>${a.source_counts?.falco||0} events</span></div><div class="kv"><b>Trivy</b><span>${esc(scan)}</span></div><div class="kv"><b>ClamAV</b><span>${a.source_counts?.clamav?'Evidence present':'No correlated evidence'}</span></div><div class="kv"><b>CrowdSec</b><span>${a.source_counts?.crowdsec?'Evidence present':'No correlated evidence'}</span></div></section><section><h3>TOP FALCO RULES</h3>${rules}</section><section><h3>RESPONSE</h3><div class="toolbar"><button onclick="evidence(${id})">Capture Evidence</button>${n?`<button onclick="scanIncident(${id},'${esc(n)}')">Scan Now</button>`:''}${top&&n?`<button onclick="markIncidentExpected(${id},'${esc(top)}')">Mark Expected</button>`:''}${n?`<button class="danger" onclick="isolateIncident(${id})">Isolate</button>`:''}${allow?`<button class="danger" onclick="createRecovery('${esc(n)}',${id})">Approved Recovery</button>`:''}<button onclick="resolveIncident(${id})">Resolve</button></div>${n&&!allow?'<div class="tiny" style="margin-top:8px">Recovery is unavailable until Approved Rebuild is enabled and Protected is off.</div>':''}</section>`;document.getElementById('drawer').classList.remove('hidden')}catch(e){toast(e.message,'error')}}async function evidence(id){let d=await api(`/api/incidents/${id}/capture-evidence`,{method:'POST'});toast('Evidence captured: '+(d.captured||[]).join(', '));load()}async function resolveIncident(id){let r=prompt('Resolution note','Resolved by operator');if(r===null)return;await api(`/api/incidents/${id}/resolve`,{method:'POST',body:JSON.stringify({resolution:r})});load()}async function sendReport(k){let d=await api(`/api/reports/send/${k}`,{method:'POST'});alert(`${k} report processed. Discord: ${d.delivery.discord}; n8n: ${d.delivery.n8n}`);load()}
