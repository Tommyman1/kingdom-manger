...
async function recommendationAction(r){if(r.action==='scan'&&r.container){await scan(r.container);return}if(r.action==='incident'&&r.incident_id){await openIncident(r.incident_id);return}if(r.action==='review-falco'&&r.container){await openBaselines(r.container);return}if(r.action==='diagnose'){await openConfidence();return}toast('Open the container security profile for details<br><button class="btn km-profile-action" onclick="openDrawer(n)">🛡 View Security Profile</button>','warn')}
