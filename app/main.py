...
def active_suppression(source: str, name: str | None, message: str = '', rule: str = '') -> dict | None:
    ...
    hay=(str(rule)+' '+str(message)).lower()
    with conn() as c:
    ...
    for r in rows:
        if (r['container_name'] or '') != name:
            continue
        needle=str(r['rule_contains'] or '').strip().lower()
...
def suppression_aware_correlation(name: str, cr: dict) -> dict:
    ...
    for sig in signals:
        ...
        sup=active_suppression(src,name,msg)
        if sup:
...
<div class="card containers"><h2 class="section-title">CONTAINER LIFE & CONTROLS</h2><div id="containers"></div></div><div class="footer"><span>🧠 Decision Engine <b class="good">Active</b></span><span>🛡 Versioned DB migrations</span><span id="footerEval">Last evaluation —</span><span>v1.11.1 · Trust Pipeline Hardening ♛</span></div></div>
<script>
